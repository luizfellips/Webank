...
<head>
    ...
    <title>Janela de Depósito</title>
</head>
        ...
        <div class="transferencia" id="transferencia">
            ...
            <form action="Deposito.php" method="post">
                <?php if (isset($_GET['Erro'])) {
                    if($_GET['Erro'] == 1){
                        echo "<p style=color:red> Um erro ocorreu no depósito.</p>";
                    } 
                    if($_GET['Erro'] == 2){
                ...
                <?php if (isset($_GET['Sucesso'])) {
                    echo "<p style=color:blue> Sucesso no depósito! </p>";
                }
                ?>
                <p>Janela de depósito</p>

                <div class="form-block">
                    <input type="text" name="NumeroConta" placeholder="Número da sua conta" required
                        autofocus>
                    <input type="number" name="Quantia" step="any" placeholder="Quantia a depositar" required>
                </div>
...
if (isset($_SESSION["UsuarioID"]) && isset($_POST["NumeroConta"]) && isset($_POST["Quantia"])) {
    ...
    try {
        ...
        if ($UsuarioAtual->Depositar($NumeroConta, $Quantia)) {
            header("Location: Deposito.php?Sucesso=1");
        } else {
            header("Location: Deposito.php?Erro=1");
        }

    } catch (Exception $err) {
        header("Location: Deposito.php?Erro=2");
    }
}
